...
func (maker *PasetoMaker) CreateToken(userUuid string, duration time.Duration, role string) (string, *Payload, error) {
	payload, err := NewPayload(userUuid, duration, role)
	if err != nil {
		return "", payload, err
	}
